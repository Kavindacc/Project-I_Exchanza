...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if(!empty($itemname) && !empty($price) && !empty($color) && !empty($description) && !empty($category) && !empty($subcategory) && !empty($condition)&& !empty($userid)){
        ...
        if(is_numeric($condition)){
            $scondition=filter_var($condition,FILTER_SANITIZE_NUMBER_INT);
        }
        else{
            $errors[] = "Condition can only number.";
        }
    ...
    if(empty($errors) && $filePath !== null || $filePatho !== null){
        ...
        if($item->addItemForThrifting($con)){
            ...
            header("Location:../view/thrift.php?error=Item Not added.");
            exit();
        }
    }
    else{
        ...
        foreach ($errors as $error) {
            header("Location: ../view/thrift.php?error=$error");
            exit();
        }
    }
