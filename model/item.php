...
class Thrift extends Item
{
    ...
    public function loaditemDetails($con,$pid)
    {
        try {
            $sql = "SELECT * FROM thrift t JOIN item i ON t.item_id = i.itemid WHERE item_id = ?"; // Adjust query to include size
            $pstmt = $con->prepare($sql);
            $pstmt->bindValue(1, $pid); // Bind the size parameter
            $pstmt->execute();
            $rows = $pstmt->fetch(PDO::FETCH_OBJ);
            return $rows;
        } catch (PDOException $e) {
            echo "Error: " . $e->getMessage();
        }
